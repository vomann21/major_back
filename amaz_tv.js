...
const scrapeLaptopData = async (url) => {
    try {
        ...
        products.each((index, product) => {
            ...
            const price = $(product).find('a-offscreen').text(); // Assuming rating is available
        ...
        if ($('.s-pagination-item.s-pagination-next.s-pagination-button.s-pagination-separator').length > 0 && pages<500) {
...
const startUrl = 'https://www.amazon.in/s?k=Tv&crid=2XM56Z01XI6KH&sprefix=tv%2Caps%2C260&ref=nb_sb_noss_1';
...
scrapeLaptopData(startUrl)
    .then((data) => {
        ...
        fs.writeFile('amaz_tv.csv', csv, (err) => {
            if (err) throw err;
