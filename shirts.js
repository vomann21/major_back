...
const scrapeAmazonSmartphones = async () => {
    try {
        const response = await axios.get('https://www.amazon.com/s?k=shirts&crid=51JRAZB57Q6Q&sprefix=shir%2Caps%2C759&ref=nb_sb_noss_2', {
        headers: {
        ...
        const existingData = JSON.parse(fs.readFileSync('amazon.json'))
        smartphones = {...existingData, ...smartphones};
        fs.writeFile('amazon_shirts.json',JSON.stringify(smartphones),(err)=>{
            if(err) throw err;
