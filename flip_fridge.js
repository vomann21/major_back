...
const scrapeLaptopData = async (url) => {
    try {
        ...
        products.each((index, product) => {
            ...
            const price = $(product).find('._1_WHN1').text(); // Assuming rating is available
...
const startUrl = 'https://www.flipkart.com/search?q=fridges&otracker=search&otracker1=search&marketplace=FLIPKART&as-show=on&as=off';
...
scrapeLaptopData(startUrl)
    .then((data) => {
        ...
        fs.writeFile('flip_fridgess.csv', csv, (err) => {
            if (err) throw err;
